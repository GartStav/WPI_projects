...
if __name__ == "__main__":
    ...
    while 1:
        ...
        while 1:
            ...
            #compute fprimeprime just the optimization force first 
            fprimeprime = 0.; #np.mat([[0., 0.], [0., 0.]]); ###YOUR CODE HERE### 

            #compute the first and second derivatives from each hyperplane and aggregate
            for j in range(0,numplanes):
                #fprime_for_plane_j should be a 2x1 matrix for our problem, it should be in terms of a,b, and x
                fprime_for_plane_j = a[:, j]/(-a[:,j].T*x + b[j]); ###YOUR CODE HERE### 
                
                #fprimeprime_for_plane_j should be a 2x2 matrix for our problem, it should be in terms of a,b, and x
                fprimeprime_for_plane_j = a[:, j]*a[:, j].T/((-a[:,j].T*x + b[j])*(-a[:,j].T*x + b[j])); ###YOUR CODE HERE### 
            ...
            plt.plot(x[0,0],x[1,0], 'gx');
            #plt.draw(); #you can comment this in but it will slow things down a lot
