...
FQueue fqueue, pqueue;




volatile int base = 0;
volatile int next_sn = 0;
int expected_sn = 0;
int new_timer = 0;
int new_sr = 0;
int fp_w;
...
void fqueue_debug_print(FQueue* queue) {
	if (fqueue_length(queue) > 0) {
		...
		fqueue_head(queue)->sn, fqueue_tail(queue)->sn,
...
void sender_handler() {
	...
	int sent_frames_len = next_sn - base;
	int counter = 0;
	while((fqueue.length != 0) && (fqueue.length > sent_frames_len)) {
		...
		if(retransmission_mode == 0)
			tmp_sn = PMOD(next_sn, WINDOWSIZE);
		else if(retransmission_mode == 1)
			tmp_sn = PMOD(next_sn, 5);
			Frame *f = fqueue_index(&fqueue, tmp_sn);
			udt_send(f, FRAMESIZE);
			di.data_amount += (header_size+f->nbuffer);
			next_sn++;
			printf("After sending, next_sn = %d, base = %d, length = %d \n",
			next_sn, base, fqueue.length);
			fflush(stdout);
			sent_frames_len = next_sn-base;
...
void receiver_handler(int sockfd) {
	...
	if (f.type == ACK) {
		printf("ACK INFO: sn: %d base: %d\n", f.sn, base);
		di.ack_recved_num++;
		while(f.sn >= base) {
			gettimeofday(&tv, NULL);
			double time2_in_mill = (tv.tv_sec) * 1000 + (tv.tv_usec) / 1000 ;
			di.time_required += (time2_in_mill - time1_in_mill);
			if(fqueue.length == 0) break;
			if(retransmission_mode == 0) {
				base++;
				fqueue_pop(&fqueue);
		...
		if(f.sn == expected_sn) {
			...
			p->type = f.type;
			//send back ACK
			send_acknowledge(f.sn, sockfd);
			expected_sn++;
		} else if(f.sn < expected_sn){
			gettimeofday(&tv, NULL);
			double time2_in_mill = (tv.tv_sec) * 1000 + (tv.tv_usec) / 1000 ;
			di.time_required += (time2_in_mill - time1_in_mill);
			di.dup_frame_recved_num++;
			send_acknowledge(expected_sn-1, sockfd);
		}
	}
}
...
void DataLinkRecv() {
	...
	while(pqueue.length != 0) {
		...
		if(p->type == FILE_STARTER) {
			...
			fflush(stdout);
			if ((fp_w = open(p->buffer, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) < 0) {
			...
			int content;
			if((content = write(fp_w, p->buffer, p->nbuffer)) < 0) {
...
static void go_back_N() {
	...
	int index;
	for(index = base; index < next_sn; index++) {
